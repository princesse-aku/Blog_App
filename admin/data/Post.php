...
function getUserByID($conn, $id){
   $sql = "SELECT id, full_name, username FROM users WHERE id=?";
   $stmt = $conn->prepare($sql);
   $stmt->execute([$id]);
...
function search($conn, $key){
    ...
    $sql = "SELECT * FROM posts WHERE post_title LIKE ? OR post_text LIKE ? ORDER BY id DESC";
    $stmt = $conn->prepare($sql);
    $stmt->execute([$key, $key]);
